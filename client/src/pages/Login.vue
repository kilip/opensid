  ...
  <div class="lg:flex">
    <div class="lg:w-1/2 xl:max-w-screen-sm">
      <div class="py-12 bg-indigo-100 lg:bg-white flex justify-center lg:justify-start lg:px-12">
        <div class="cursor-pointer flex items-center">
          ...
          <div class="text-2xl text-indigo-800 tracking-wide ml-2 font-semibold">
            {{ config ? config['namaDesa']: 'OpenSID'}}
            <span class="block text-xs justify-center font-semibold">
              Kec. {{ config['namaKecamatan']}}, Kab. {{config['namaKabupaten']}}, Prov. {{ config['namaProvinsi']}}
            </span>
          </div>
        </div>
      </div>
      <div class="mt-10 px-12 sm:px-24 md:px-48 lg:px-12 lg:mt-16 xl:px-24 xl:max-w-2xl">
        ...
        <div class="mt-12">
          <div v-if="error">
            <h6 class="text-sm">Gagal Login</h6>
...
export default {
  ...
  methods: {
    ...
    async doLogin(){
      ...
      if(this.loggedIn){
        const redirect = this.$route.params['redirect'] || '/dashboard';
